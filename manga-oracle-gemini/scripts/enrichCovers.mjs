...
async function searchGoogleBooks(entry) {
  ...
  const query = [`intitle:${entry.title_ja || entry.title_en}`, entry.author && `inauthor:${entry.author}`, "1巻 漫画"].filter(Boolean).join(" ");
...
async function searchRakuten(entry) {
  ...
  url.searchParams.set("title", `${entry.title_ja || entry.title_en} 1`);
...
function scoreCandidate(entry, candidate) {
  ...
  if (/漫画|コミック|comic|comics|ジャンプ|マガジン|サンデー|花とゆめ|りぼん|マーガレット/i.test(`${candidate.title} ${candidatePublisher} ${candidateCategories}`)) {
    score += 10;
    notes.push("comic");
  }
